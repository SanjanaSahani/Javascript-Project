...
window.addEventListener("DOMContentLoaded", function () {
    displayMenu(recipeCard)

})

function displayMenu(menuItems) {
    let displayCard = menuItems.map((items) => {
        return `
<article class="recipe">
<div class="recipe-box">
    <img src=${items.img} alt=${items.category}>
</div>
<div class="recipe-content">
    <div class="menuName">
        <h1 class="recipe-title">${items.title}</h1>
        <p class="recipe-price">
            <span class="price">${items.price}</span>
        </p>  
    </div>
    <div class="underline"></div>
    <p class="recipe-desc">${items.desc}</p>
</div>
</article>
`
    })
...
btn.forEach((button) => {
    button.addEventListener('click', function (e) {
        ...
        let MenuCategory = recipeCard;
        
        if (category !== "All") {
            MenuCategory = recipeCard.filter((menuItems) => {
